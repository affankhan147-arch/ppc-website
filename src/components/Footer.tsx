    ...
    <footer className="border-t border-white/10 bg-[#0B1917] text-white">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 md:grid-cols-[1.35fr_1fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-2 font-serif text-xl text-white">Emergency plumbing service for stressful moments.</p>
          <p className="mt-3 max-w-md text-sm leading-6 text-white">{siteConfig.disclosure}</p>
          
            <a
            href={phoneConfig.href}
            data-phone-location="footer"
            className="mt-5 inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#F0B429] px-5 py-3 text-sm font-black text-white shadow-lg transition hover:bg-[#d99f1f]"
            aria-label={`Call ${phoneConfig.display} from footer`}
          >
            <PhoneCall className="h-4 w-4" aria-hidden="true" />
            Call {phoneConfig.display}
          </a>
          <p className="mt-5 rounded-xl border border-white/10 bg-white/[.05] p-4 text-sm font-semibold leading-6 text-white">
