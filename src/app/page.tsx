          ...
          <article className="max-w-3xl">
            ...
            <div className="trust-ribbon mt-10 grid gap-1 rounded-2xl p-3 sm:grid-cols-3">
              {["Emergency phone line", "DFW plumbing service", "Clear next steps"].map((item) => (
                <div key={item} className="flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-bold text-white">
                  <CheckCircle2 className="h-4 w-4 flex-none " aria-hidden="true" /> {item}
                </div>
              ))}
            </div>
          </article>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {urgentCards.map(({ title, text, icon: Icon, href }) => (
              <Link key={title} href={href} className="premium-card group p-5">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-sm font-black ">See safe next steps <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" aria-hidden="true" /></span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="page-shell">
        <section className="content-section overflow-hidden p-0">
          <div className="grid lg:grid-cols-[.88fr_1.12fr]">
            ...
            <div className="p-7 sm:p-10 lg:p-12">
              ...
              <p className="mt-5 text-lg leading-8 text-white">A clear description helps identify whether the problem involves one fixture, a wider drain or sewer line, a supply pipe, or a water heater.</p>
              <div className="mt-6 grid gap-3">
                {["Which fixture changed first", "Whether water is clean or contaminated", "What shutoff or cleanout is safely accessible", "Whether another fixture reacts at the same time"].map((item) => (
                  <div key={item} className="flex items-center gap-3 rounded-xl bg-[#0F1F1D] px-4 py-3 font-bold text-white"><SearchCheck className="h-5 w-5 flex-none " aria-hidden="true" />{item}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <section id="services" className="content-section">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <Link href="/services/24-hour-emergency-plumber" className="inline-flex items-center gap-2 font-black ">Emergency plumber overview <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
          </div>
            ...
            {featuredServices.map((service) => (
              <Link key={service.slug} href={`/services/${service.slug}`} className="premium-card group overflow-hidden">
                ...
                <div className="p-5">
                  ...
                  <p className="mt-2 line-clamp-3 leading-7 text-white">{service.shortAnswer}</p>
                  <span className="mt-4 inline-flex items-center gap-2 text-sm font-black ">Explore this service <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" aria-hidden="true" /></span>
                </div>
              </Link>
            ))}
          </div>
        ...
        <section className="content-section dark-band">
          <div className="mx-auto max-w-5xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-white">Share the problem, your DFW location, and the urgency by phone so the right plumbing service can be identified without a long form.</p>
          </div>
          <div className="mx-auto mt-9 grid max-w-5xl gap-4 md:grid-cols-3">
            {["Describe the affected fixture and urgency", "Share the DFW city or ZIP", "Discuss the right service and next step"].map((step, index) => (
              <div key={step} className="rounded-2xl border border-white/15 bg-white/[.07] p-5 text-left"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#F0B429] font-black ">{index + 1}</span><p className="mt-4 font-bold leading-7 text-white">{step}</p></div>
            ))}
          </div>
        </section>
        ...
        <section className="content-section">
          <div className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-3xl text-lg leading-8 text-white">From the first safety step to choosing the right service, Plumbing Hands keeps the process focused on the problem that needs attention.</p>
          </div>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {["Emergency-ready guidance", "Service organized by problem", "Coverage across Dallas-Fort Worth"].map((item) => (
              <div key={item} className="premium-card p-6"><CheckCircle2 className="h-8 w-8 " aria-hidden="true" /><h3 className="mt-4 text-xl font-black text-white">{item}</h3></div>
            ))}
          </div>
        </section>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {["Explain what is happening", "Share your DFW location", "Confirm the next step"].map((title, index) => (
              <article key={title} className="overflow-hidden rounded-3xl border border-white/10 bg-[#16302C] shadow-sm">
                ...
                <div className="p-5">
                  <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#F0B429] text-base font-black ">{index + 1}</span>
        ...
        <section className="content-section soft-band">
          <div className="grid gap-8 lg:grid-cols-[.85fr_1.15fr]">
            <div>
              ...
              <p className="mt-4 text-lg leading-8 text-white">Choose your city to find emergency plumbing service for leaks, drains, sewers, toilets, pipes, and water heaters.</p>
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-[#16302C] p-5 shadow-sm"><p className="text-4xl font-black text-white">{cities.length}</p><p className="mt-1 text-sm font-bold text-white">DFW service areas</p></div>
                <div className="rounded-2xl bg-[#16302C] p-5 shadow-sm"><p className="text-4xl font-black text-white">{services.length}</p><p className="mt-1 text-sm font-bold text-white">urgent plumbing services</p></div>
              </div>
              <Link href="/cities" className="mt-6 inline-flex items-center gap-2 font-black ">Find service in your city <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {cities.slice(0, 12).map((city) => <Link key={city.slug} href={`/cities/${city.slug}`} className="premium-card flex items-center gap-3 p-4 font-black text-white"><MapPin className="h-5 w-5 " aria-hidden="true" />{city.name}</Link>)}
            </div>
          </div>
        </section>
            ...
            {costGuides.map((guide, index) => (
              <Link key={guide.slug} href={`/cost-guides/${guide.slug}`} className="premium-card group overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <p className="mt-3 leading-7 text-white">{guide.directAnswer}</p>
                </div>
              </Link>
            ))}
          </div>
            ...
            {problems.slice(0, 6).map((problem, index) => (
              <Link key={problem.slug} href={`/problems/${problem.slug}`} className="premium-card group overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <p className="mt-3 leading-7 text-white">{problem.directAnswer}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <section className="content-section dark-band text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-white">Tell us the plumbing problem and your Dallas-Fort Worth location. The emergency line is ready for active leaks, backups, toilet, pipe, sewer, and water-heater service needs.</p>
